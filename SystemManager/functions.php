...
{
	$rtrnval="";
	//return "SELECT $rtrnfild FROM $table WHERE $srcfild='$srcval' $morecond";
...
function GetDebitCreditRupees($LedgerHead,$fdtt,$flag,$con)
{
	if($fdtt==""){$dt="WHERE PaymentDate<'$fdtt'";}else{if($flag==0){$dt="WHERE PaymentDate<='$fdtt'";}else{$dt="WHERE PaymentDate<'$fdtt'";}}
	$sql=mysqli_query($con,"SELECT (SUM(IF(DebitLedger='$LedgerHead', TransactionAmount, 0)) - SUM(IF(CreditLedger='$LedgerHead', TransactionAmount, 0))) AS modbal  FROM main_drcr $dt");
	while($R=mysqli_fetch_array($sql))
	{
		$modbal=$R['modbal'];
	}
...
{
	return '<img  alt="testing" src="BarCode.php?text='.$data.'&codetype=code128&size=30&print=true"/>';
}
?>