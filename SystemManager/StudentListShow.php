...
$sql=mysqli_query($con,"SELECT * FROM $tblnm WHERE sl>0 $CurrentSession1 $CurrentClass1 $CurrentSection1 $Gender1 $Caste1 $Religion1 $AllSearch $User ORDER BY sl")or die(mysqli_error($con));
$rcntttl=mysqli_num_rows($sql);
$rcnt=$rcntttl;
$sql=mysqli_query($con,"SELECT * FROM $tblnm WHERE sl>0 $CurrentSession1 $CurrentClass1 $CurrentSection1 $Gender1 $Caste1 $Religion1 $AllSearch $User ORDER BY sl LIMIT $start,$ps") or die(mysqli_error($con));
