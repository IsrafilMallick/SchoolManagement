...
$EnquiryID=isset($_POST['EnquiryID']) ? $_POST['EnquiryID'] : '';
$EnquirySession=isset($_POST['EnquirySession']) ? $_POST['EnquirySession'] : '';
$EnquiryDate=isset($_POST['EnquiryDate']) ? date('Y-m-d', strtotime($_POST['EnquiryDate'])) : '';
$EnquiryCalss=isset($_POST['EnquiryCalss']) ? $_POST['EnquiryCalss'] : '';
...
$StudentName=isset($_POST['StudentName']) ? NameCase($_POST['StudentName']) : '';
...
$FatherName=isset($_POST['FatherName']) ? NameCase($_POST['FatherName']) : '';
...
$MotherName=isset($_POST['MotherName']) ? NameCase($_POST['MotherName']) : '';
...
$GuardianName=isset($_POST['GuardianName']) ? NameCase($_POST['GuardianName']) : '';
...
if($EnquirySession==''||$EnquiryDate==''||$EnquiryCalss==''||$StudentName==''||$Gender==''||$FatherName==''||$FatherMobile==''||$ResidentVillage==''||$ResidentPostOffice==''||$ResidentPoliceStation==''||$ResidentDistrict==''||$ResidentState==''||$ResidentPin=='')
{
...
{
	$sql=mysqli_query($con,"SELECT sl FROM $tblnm WHERE EnquirySession='$EnquirySession' AND StudentName='$StudentName' AND BirthDate='$BirthDate' AND FatherName='$FatherName' AND FatherMobile='$FatherMobile' $sl1");
	if(mysqli_num_rows($sql)==0)
	{
		if($sl=="")
		{
			//E20181901001 ---> Abbreviation+Session+Class+Increment
			$increment=0;
			$abbrev='E'.$EnquirySession.substr($EnquirySession+1,2,4).str_pad($EnquiryCalss, 2, '0', STR_PAD_LEFT);
			$sql=mysqli_query($con,"SELECT EnquiryID FROM $tblnm WHERE EnquirySession='$EnquirySession' AND EnquiryID LIKE '$abbrev%' ORDER BY sl DESC LIMIT 0,1");
			while($R=mysqli_fetch_array($sql))
			{
				$EnquiryID=$R['EnquiryID'];
				$increment=substr($EnquiryID,9,3);
			}
			...
			{
				$increment+=1;
				$EnquiryID=$abbrev.str_pad($increment, 3, '0', STR_PAD_LEFT);
				$sql=mysqli_query($con,"SELECT sl FROM $tblnm WHERE EnquiryID='$EnquiryID'");
				$count=mysqli_num_rows($sql);
			}
			mysqli_query($con,"INSERT INTO $tblnm (EnquiryID, EnquirySession, EnquiryDate, EnquiryCalss, AdmissionSession, AdmissionClass, AdmissionDate, CurrentSession, CurrentClass, CurrentSection, StudentName, BirthDate, Gender, Caste, Religion, AadharNo, FatherName, FatherQualification, FatherOccupation, FatherMobile, MotherName, MotherQualification, MotherOccupation, MotherMobile, GuardianName, GuardianQualification, GuardianOccupation, GuardianMobile, GuardianRelation, PermanentVillage, PermanentPostOffice, PermanentPoliceStation, PermanentDistrict, PermanentState, PermanentPin, ResidentVillage, ResidentPostOffice, ResidentPoliceStation, ResidentDistrict, ResidentState, ResidentPin, BankName, BranchName, BranchAddress, BranchIFSC, BranchMICR, AccountNo, edt, edttm, eby) VALUES('$EnquiryID', '$EnquirySession', '$EnquiryDate', '$EnquiryCalss', '$AdmissionSession', '$AdmissionClass', '$AdmissionDate', '$CurrentSession', '$CurrentClass', '$CurrentSection', '$StudentName', '$BirthDate', '$Gender', '$Caste', '$Religion', '$AadharNo', '$FatherName', '$FatherQualification', '$FatherOccupation', '$FatherMobile', '$MotherName', '$MotherQualification', '$MotherOccupation', '$MotherMobile', '$GuardianName', '$GuardianQualification', '$GuardianOccupation', '$GuardianMobile', '$GuardianRelation', '$PermanentVillage', '$PermanentPostOffice', '$PermanentPoliceStation', '$PermanentDistrict', '$PermanentState', '$PermanentPin', '$ResidentVillage', '$ResidentPostOffice', '$ResidentPoliceStation', '$ResidentDistrict', '$ResidentState', '$ResidentPin', '$BankName', '$BranchName', '$BranchAddress', '$BranchIFSC', '$BranchMICR', '$AccountNo', '$edt', '$edttm', '$eby')") or die(mysqli_error($con));
			$msg="Submitted Successfully. . . ! ! !";
			$returnpage="document.location='AdmissionEnquiryPrint.php?EnquiryID=$EnquiryID';";
		}
		else
		{
			...
			while($R=mysqli_fetch_array($sql))
			{
				$EnquiryID1=$R['EnquiryID'];
				$EnquirySession1=$R['EnquirySession'];
				$EnquiryDate1=$R['EnquiryDate'];
				$EnquiryCalss1=$R['EnquiryCalss'];
			...
			$tblsl=$sl;
			$ufnm='sl';
			if($EnquirySession!=$EnquirySession1)
			{
				$fldnm='EnquirySession';
				$old_val=$EnquirySession1;
				$new_val=$EnquirySession;
				EditLog($tblnm,$tblsl,$ufnm,$fldnm,$old_val,$new_val,$edt,$edttm,$eby,$con);
			}
			
			if($EnquiryDate!=$EnquiryDate1)
			{
				$fldnm='EnquiryDate';
				$old_val=$EnquiryDate1;
				$new_val=$EnquiryDate;
				EditLog($tblnm,$tblsl,$ufnm,$fldnm,$old_val,$new_val,$edt,$edttm,$eby,$con);
			}
			
			if($EnquiryCalss!=$EnquiryCalss1)
			{
				$fldnm='EnquiryCalss';
				$old_val=$EnquiryCalss1;
				$new_val=$EnquiryCalss;
				EditLog($tblnm,$tblsl,$ufnm,$fldnm,$old_val,$new_val,$edt,$edttm,$eby,$con);
			}
