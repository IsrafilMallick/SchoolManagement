...
if($batch==""){$batch1="";}else{$batch1="AND batch LIKE '%$batch%'";}
...
$sql=mysqli_query($con,"SELECT * FROM $tblnm WHERE sl>0 $sesn1 $course1 $batch1 $day1 $time1") or die(mysqli_error($con));
$rcntttl=mysqli_num_rows($sql);
$rcnt=$rcntttl;
$sql=mysqli_query($con,"SELECT * FROM $tblnm WHERE sl>0 $sesn1 $course1 $batch1 $day1 $time1 ORDER BY sl ASC LIMIT $start,$ps") or die(mysqli_error($con));
...
while($R=mysqli_fetch_array($sql))
{
	...
	<tr>
		...
		<td id="tblbody">
        <a href="BatchSetup.php?sl=<?=$sl;?>"><i class="fa fa-edit fa-lg" title="Click to Edit" style="cursor:pointer;"></i></a>
        <span id="show<?=$sl;?>">
